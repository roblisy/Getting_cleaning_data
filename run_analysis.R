...
complete <- cbind(y_complete, subject_complete, x_complete)
...
names(complete) <- c('activity_code', 'activity_description', 'subject_ID', nicenames)

# drop that first column, it's not necessary.
complete <- complete[,-1]

# Question 5 - make a tidy data set.
complete_tidy <- melt(data = complete,
                        id.vars = c("activity_description", "subject_ID"))

# average out the subject and activity.
complete_tidy_mean <- dcast(data = complete_tidy, 
